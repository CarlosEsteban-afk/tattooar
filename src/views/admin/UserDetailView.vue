<template>
    <div class="min-h-screen bg-gray-50 flex flex-col md:flex-row">
        <div class="flex-1 flex flex-col w-full sm:pl-64">
            <div class="p-4 md:p-4 pt-6 md:pt-6 sm:pt-6 justify-center">
                <AdminSidebar @logout="logout" />
                <TopBar title="Estadísticas usuarios" :notificationCount="3" />
                <br />
                <div class="flex justify-center items-center">
                    <div class="w-full bg-white rounded-md shadow-2xl border border-gray-200 p-6">
                        <div v-if="user" class="flex flex-col items-center">
                            <!-- Imagen y datos principales -->
                            <div class="relative mb-4">
                                <img :src="user.imagen || defaultImg" alt="Profile"
                                    class="w-36 h-36 rounded-full object-cover border-4 border-purple-400 shadow-lg" />
                                <span class="absolute bottom-2 right-2 px-2 py-1 rounded-full text-xs font-semibold"
                                    :class="user.estado === 'Activo' ? 'bg-green-100 text-green-700 border border-green-300' : 'bg-red-100 text-red-700 border border-red-300'">
                                    {{ user.estado || 'Sin estado' }}
                                </span>
                            </div>
                            <h2 class="text-2xl md:text-3xl font-bold text-center mb-1 text-[#2E076B] tracking-tight">
                                {{ user.rol === 'Tatuador' ? user.nombre : user.alias }}
                            </h2>
                            <p class="text-gray-600 text-center font-medium">{{ user.nombre }} {{ user.apellido }}</p>
                            <p class="text-sm text-gray-400 text-center mb-3">{{ user.correo }}</p>
                            ...
                            <div class="flex flex-col sm:flex-row gap-3 w-full justify-center mb-6">
                                <button @click="toggleUserStatus" class="flex-1 flex items-center justify-center px-4 py-2 rounded-lg font-semibold transition
                        duration-150" :class="user.estado === 'Activo'
                            ...
                            : 'bg-green-500 hover:bg-green-600 text-white shadow'">
                                ...
                                <button @click="goToEditUser(user.id)"
                                    class="flex-1 flex items-center justify-center px-4 py-2 rounded-lg bg-[#2E076B] text-white hover:bg-purple-900 gap-2 font-semibold shadow transition duration-150"
                            ...
                            <button @click="goBack"
                                class="mt-2 px-4 py-2 bg-purple-100 text-[#2E076B] rounded-lg hover:bg-purple-200 font-semibold w-full transition duration-150">
                                Volver
                            </button>

                            <!-- Detalle de Tatuador -->
                            <div v-if="user.rol === 'Tatuador'" class="w-full mt-10">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                                    <div>
                                        <h3 class="font-semibold mb-1 text-[#2E076B]">Estilos</h3>
                                    ...
                                    <div>
                                        <h3 class="font-semibold mb-1 text-[#2E076B]">Ciudad(es)</h3>
                                        <div class="flex flex-wrap gap-2">
                                    ...
                                    <div class="md:col-span-2">
                                        <h3 class="font-semibold mb-1 text-[#2E076B]">Contacto</h3>
                                        <div class="flex gap-3 items-center flex-wrap">
                                            <a v-if="user.whatsapp" :href="`https://wa.me/${user.whatsapp}`"
                                                target="_blank"
                                                class="text-green-600 flex items-center hover:underline">
                                            ...
                                            <a v-if="user.instagram" :href="`https://instagram.com/${user.instagram}`"
                                                target="_blank" class="text-pink-500 flex items-center hover:underline">
                                                <svg class="w-5 h-5 mr-1" fill="currentColor" viewBox="0 0 24 24">
                                                    <path
                                ...
                                <!-- Diseños disponibles -->
                                <div class="mb-8">
                                    <h3 class="font-semibold mb-2 text-[#2E076B]">Diseños disponibles</h3>
                                    <div class="max-h-[420px] overflow-y-auto">
                                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                            <div v-for="diseño in user.diseños || []" :key="diseño.id"
                                                class="bg-[#7B6EAD] rounded-xl p-2 flex flex-col items-center shadow hover:scale-105 transition-transform duration-150">
                                                <img :src="diseño.imagen" alt="Diseño"
                                                    class="w-full h-40 object-cover rounded mb-2" />
                                                <span class="text-xs text-white font-medium">{{ diseño.nombre }}</span>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <!-- Portafolio -->
                                <div>
                                    <h3 class="font-semibold mb-2 text-[#2E076B]">Portafolio</h3>
                                    <div class="max-h-[420px] overflow-y-auto">
                                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
...
async function toggleUserStatus() {
    const nuevoEstado = user.value.estado === 'Activo' ? 'Inactivo' : 'Activo'
    const confirmMsg = user.value.estado === 'Activo'
        ? `¿Estás seguro de que quieres desactivar a ${user.value.nombre}?`
        : `¿Deseas activar nuevamente a ${user.value.nombre}?`
