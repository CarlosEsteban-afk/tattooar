...
<template>
  <header class="flex justify-between items-center bg-white shadow-sm border border-gray-300 px-6 py-4 rounded-md">
    ...
    <div class="flex items-center gap-6">
      <div class="relative">
        <div class="flex">
          ...
          <div v-if="showMenu" class="absolute right-12 mt-8 w-64 bg-white border rounded shadow-lg z-50">
            <div v-if="notifications.length === 0" class="p-4 text-gray-500 text-sm">
              No tienes notificaciones
            </div>
            <ul v-else>
              <li v-for="n in notifications" :key="n.id" class="hover:bg-gray-100 cursor-pointer px-4 py-2 text-sm"
                @click="goToNotification(n)">
                <img :src="getImgUrl(n.urlImagen)" alt="Notification Image"
